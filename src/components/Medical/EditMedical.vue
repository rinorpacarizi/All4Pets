  ...
  <div style="padding-left: 120px">
    ...
    >
     <el-form-item style="width:340px;" label="Pet Name" prop="name">
       ...
       <el-form-item label="Medical Type" prop="type">
        <el-select v-model="ruleForm.type">
          <el-option
            v-for="type in typesM"
            v-bind:key="type.id"
      ...
      <el-form-item label="Medical notes" prop="notes">
...
export default {
  ...
  methods: {
    async submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.axios.put(
            "http://localhost:3000/medical/" + this.activeId,
            this.ruleForm
          );
  ...
  async mounted() {
    await Vue.axios.get("http://localhost:3000/typesM").then((res) => {
      this.typesM = res.data;
    });
    await Vue.axios
      .get("http://localhost:3000/medical/" + this.activeId)
      .then((res) => {
